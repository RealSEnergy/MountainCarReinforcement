...
def CreateModel(layers, learning_rate):
	...
	if len(layers) == 0:
		model.add(keras.layers.Dense(3, input_dim=2, activation="linear", name=("lr_{}".format(learning_rate))));
	else:
		model.add(keras.layers.Dense(layers[0], input_dim=2, name=("lr_{}".format(learning_rate))));
